...
// ... (Interface tetep utuh sesuai kodingan lo)
interface VolunteeringItem { id: string; organization: string; role: string; description: string; impact: string[]; tags: string[] }
interface AchievementItem { id: string; title: string; date: string; category: string; description: string; details: string[]; tags: string[] }

interface AccordionItemProps {
  id: string
  title: string
  subtitle?: string
  logo?: string // Tambahan untuk logo tiap item
  summary: string
  details?: string[]
  tags?: string[]
  images?: string[]
  ...
  detailLink?: string
  category?: string
  variants?: any // Untuk animasi masuk (fade in up)
}

export function AccordionItem({
  id, title, subtitle, logo, summary, details = [], tags = [], images = [],
  startDate, endDate, volunteering = [], achievements = [], skills = [],
  supportingDocuments = [], isPlaceholder = false, expandedPreview = false,
  detailLink, category, variants
}: AccordionItemProps) {
  ...
  return (
    ...
    >
      {/* HEADER: Gabungan Collapsed & Expanded biar gak nge-blink */}
      <button
          ...
          <div className="flex w-full flex-col gap-3 sm:gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex-1 space-y-1">
              <div className="flex items-baseline gap-2 flex-wrap">
                ...
                {isPlaceholder && (
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-primary/10 text-primary">Skeleton</span>
                )}
              </div>
              ...
              {/* Summary cuma muncul pas collapsed (sesuai kodingan awal lo) */}
              {!isExpanded && (
                <p className="text-sm mt-3 text-foreground/70 line-clamp-2">{summary}</p>
              )}
            </div>
      ...
      {/* CONTENT: Pakai AnimatePresence biar smooth expand nya */}
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            <div className="p-6 bg-background/50 grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {images.length > 1 && (
                      <div className="flex items-center justify-center gap-2">
                        {images.map((_, index) => (
                          <button key={index} onClick={(e) => { e.stopPropagation(); setImageIndex(index); }}
                            className={cn("h-2 rounded-full transition-all", index === imageIndex ? "bg-primary w-8" : "bg-primary/30 w-2")}
                          />
                        ))}
                      </div>
                    )}
                ...
                {volunteering.length > 0 && (
                  <NestedAccordionSection title="Volunteering" items={volunteering.map(v => ({ id: v.id, title: v.organization, subtitle: v.role, description: v.description, details: v.impact, tags: v.tags }))} variant="volunteering" />
                )}
                {achievements.length > 0 && (
                  <NestedAccordionSection title="Achievements" items={achievements.map(a => ({ id: a.id, title: a.title, subtitle: `${a.category} • ${a.date}`, description: a.description, details: a.details, tags: a.tags }))} variant="achievements" />
                )}
                ...
                {skills.length > 0 && (
                  <>
                    ...
                    <div className='flex gap-3'>
                      {[1, 2, 3, 4].map(i => <div key={i} className='bg-blue-500 w-10 h-10 rounded-full'/>)}
                    </div>
                  </>
                )}
                ...
                {category && id && (
                  <div className="flex justify-end pt-4 border-t border-foreground/10">
                    <a href={`/${category}/${id}`} className="inline-flex items-center gap-2 rounded-lg border border-primary/30 text-primary px-4 py-2 text-sm font-medium transition-all hover:bg-primary/5">
                      View Details
